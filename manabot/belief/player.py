...
from manabot.belief.tracker import ActionLikelihood, ExactRangeTracker
...
class ExactRangePlayer:
    ...
    def __init__(
        ...
        *,
        ...
        self._step_likelihood_root: Any | None = None
        self._prepared_action_type: int | None = None
    ...
    def start_game(self, env: Env, seat: int) -> None:
        ...
        self._step_likelihood_root = None
        self._prepared_action_type = None

    def prepare_step(self, env: Env, acting: int, action: int) -> None:
        if self.seat is None or acting == self.seat:
            return
        raw = env.last_raw_obs
        option = raw.action_space.actions[action]
        action_type = int(option.action_type)
        self._prepared_action_type = action_type
        self._step_likelihood_root = None
        if action_type in (0, 1) and self._pending_likelihood_root is None:
            self._pending_likelihood_root = env._engine.clone_env()
        elif action_type == 2:
            self._step_likelihood_root = env._engine.clone_env()
    ...
    def observe_step(self, env: Env, acting: int) -> None:
        ...
        after_counts, after_hand, _ = env._engine.hidden_pool_summary(self.seat)
        after_map = {int(definition): int(count) for definition, count in after_counts}
        exits = [
            definition
        ...
        if acting != self.seat and self._prepared_action_type == 2:
            public_action = PublicAction(PublicActionKind.PASS_PRIORITY)
            likelihood_root = self._step_likelihood_root
        elif acting != self.seat and exits:
            if len(exits) != 1:
                raise RangeError("one public commitment changed multiple hidden definitions")
    ...
    def act(self, env: Env, obs: dict[str, np.ndarray]) -> int:
        ...
        scores, simulations, cap_hits, installed = (
            env._engine.flat_mc_scores_for_hands(
                hands,
                world_seeds,
                self.rollouts,
                self.max_steps,
        ...
        mismatches = sum(sorted(actual) != wanted for actual, wanted in zip(installed, expected))
        if mismatches:
            self.stats.installed_hand_mismatches += mismatches
            raise RuntimeError("managym installed a different hand than the sampled range key")
        ...
        self.stats.end_to_end_seconds += time.perf_counter() - started
...
class UniformRangePlayer(ExactRangePlayer):
    def __init__(self, sims: int, *, likelihood: ActionLikelihood, **kwargs: Any) -> None:
        super().__init__(
            sims,
