...
def belief_checkpoint_fields(
    schema: BeliefEncodingSchema | None,
    *,
    count_buckets: int,
    card_vocab_size: int,
) -> dict[str, str]:
    ...
    if count_buckets <= 0:
        ...
        return {}
    if schema is None:
        raise ValueError("belief-enabled checkpoints require an exact belief schema")
...
def encode_belief(
    ...
    hand_counts: dict[str, NDArray[np.int64]] = {}
    for row_index, row in enumerate(schema.rows):
        ...
        if row.card_name not in hand_counts:
            hand_counts[row.card_name] = np.fromiter(
                (world.count(row.card_name) for world in belief.space.worlds),
                dtype=np.int64,
                count=belief.space.support_size,
            )
        counts = hand_counts[row.card_name]
        if row.hidden_zone_id == LIBRARY_ZONE_ID:
            counts = pool.get(row.card_name, 0) - counts
        if np.any(counts < 0):
            raise BeliefError(
                f"world hand count exceeds the unseen pool for {row.card_name!r}"
            )
        overflow = counts >= schema.count_buckets
        if np.any(overflow):
            count = int(counts[overflow][0])
            raise BeliefError(
                f"count {count} for {row.card_name!r} exceeds the encoding schema"
            )
        # bincount adds weights in canonical world order, preserving the
        # scalar float64 accumulation and therefore the encoding receipt.
        probabilities64[row_index] = np.bincount(
            counts, weights=belief_probabilities, minlength=schema.count_buckets
        )
    if not np.allclose(
    ...
    digest.update(
        np.asarray([belief.entropy, belief.effective_range_size], dtype="<f8").tobytes()
    )
