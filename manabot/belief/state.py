...
class ViewerHistory:
    ...
    def validate_world_space(self, space: PossibleWorldSpace) -> None:
        """Require the support and history to describe the same viewer state."""

        if self.viewer != space.viewer:
            raise BeliefError("viewer history does not match the world-space viewer")
        if self.current_revision != space.source_revision:
            raise BeliefError("viewer history revision does not match the world space")
        if self.current_viewer_state_hash != space.source_viewer_state_hash:
            raise BeliefError(
    ...
    def from_belief(
        ...
        *,
        ...
        return cls(
            belief=belief,
            update_receipt=BeliefUpdateReceipt(
                model_identity=belief.model_id,
                previous_belief=None if previous is None else previous.digest,
                viewer_history_identity=viewer_history.identity,
                consumed_history_range=consumed_history_range,
...
class CompatibleDealBeliefModel:
    ...
    def update(
        ...
        *,
        ...
        viewer_history.validate_world_space(world_space)
        ...
        return BeliefUpdate.from_belief(
            belief,
            previous=previous,
