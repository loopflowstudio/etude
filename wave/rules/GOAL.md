...
consumed by the exact-range tracker and conditional search. Checked RUL-11 and
RUL-12 evidence now covers all 62 public commitments in the selected
132-Command trace with zero exercised `RulesProviderGap`; unadmitted boundaries
still fail closed. The near-term provider pressure reported back by the running
calibration path is instead repeated support construction: likelihood updates
must reuse one exact root-bound possible-world materializer across bounded
inference batches without re-enumerating the world space, retaining all
identity, isolation, and stale-root checks. Closing that cost seam unblocks the
live belief/advice consumer and outranks new content breadth.
