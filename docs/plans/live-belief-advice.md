...
## Carry-forward status (2026-09-24)

This is the unfinished ETU-14/ETU-21 design retained from scratch. It is not a
release receipt. The checkout now implements `DecisionAddressV2`, pending-root
promotion, the managym conditioning-index binding, and a selected live tracked
posterior resolver. `tests/etude/test_live_advice.py` covers those three seams.
The production `/api/advice` route still calls the fixture provider; the
participant-authenticated live provider, full UI connection, fresh live/Study
byte parity, and concurrency/performance gates below are not demonstrated.
Descriptions of those already-built seams as missing are the original design
baseline, not current status.

Checkpoint availability and the quoted performance observations are historical
measurements from the design investigation, not revalidated deployment facts.
The belief-forming branch changes checkpoint-loader identity and removes
positional condition model fields. Revalidate the retained likelihood/evaluator
bytes against the current ABI before resuming; if incompatible, freeze new
models under an explicit new contract rather than aliasing or porting old
weights or rewriting frozen evidence. Missing or mismatched artifacts must
leave advice unavailable while ordinary Commands continue.

Linear reconciliation is currently blocked by the repository PM migration
owned by PRD-44. The offline cache still has ETU-14 and ETU-21 open; refresh it
before editing either task. Preserve their end-to-end finish lines.
